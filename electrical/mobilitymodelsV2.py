...
class unified_mobility():
    ...
    def __new__(cls, vals, impurity, dn, N_a, N_d, temp, carrier, ni):
        '''
    ...
    def Z(self, Type):
        ...
        # Done
        i = self.type_dic[Type]
        return 1. + 1. / (self.vals['c'][i] +
                          (self.vals['nref2'][i] / self.return_dopant(Type))**2.)
    ...
    def G(self, Type):
        ...
        c = self.vals['s5'] / \
            ((300. / self.temp /
              self.vals['mr'][i])**self.vals['s7'] * self.P(Type))**self.vals['s6']
        return a + b + c
    ...
    def F(self, Type):
        ...
        return (self.vals['r1'] * self.P(Type)**self.vals['r6']
                + self.vals['r2'] + self.vals['r3'] *
                self.vals['mr'][i] / self.vals['mr'][j]
                ) / (
